...
- **Qualifiers:** `none`
- **Next exact step:** executar arquitetura/crítica R8R sem contexto sobre a baseline material imutável e concluir os guards pré-aprovação.
...
- **Next path/status action:** executar as revisões R8R e os guards pré-aprovação, então solicitar `APROVADO` explícito com aceite de `RISK-HIST-01`.
